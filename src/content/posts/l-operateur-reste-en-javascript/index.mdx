...
Si on jette une œil à [la documentation MDN](https://developer.mozilla.org/fr/docs/Web/JavaScript/Reference/Operators/Remainder), on se rend compte que c'est le résultat qu'on obtient avec un opérateur `modulo` et non pas `reste`, malheureusement cet opérateur n'existe pas en javascript. Pour y remédier on peut écrire la petite fonction utilitaire proposée par la documentation :
...
function update(newValue){
    current = modulo(newValue, SPRITES.length)
    showSprite(SPRITES[current])
}
```
